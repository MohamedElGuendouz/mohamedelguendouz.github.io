...
> C'est un concept d'architecture de programmation créer par Google. Il permet de manipuler de grandes quantités de données en les distribuant dans un cluster de machines pour être traitées.
...
### D) Flume
> C'est un logicielle permettant la collecte, l'agreggation et le transfert de gros volumes de logs dont l'objectif est de gérer des pic de charge importantes. Il est l'alternative a Kafka même si ce dernier répond à d'autres problématiques (massage broker).
...
***Apache Phoenix*** offre la possible d'avoir un accès SQL aux tables HBase.  
